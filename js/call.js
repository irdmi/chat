...
async function sendSdpMessage(sdpData) {
  ...
  try {
    const resp = await fetch(PROXY_URL + '?action=write', {
      ...
      body: JSON.stringify({ file: SDP_FILE, encrypted: encrypted })
    });
...
async function loadRemoteSdpMessages() {
  try {
    const url = PROXY_URL + '?action=read&file=' + encodeURIComponent(SDP_FILE) + '&t=' + Date.now();
    ...
    updateStatus('No new messages', 'disconnected');
    
  } catch (err) {
    console.error('Load SDP error:', err);
    updateStatus('Load failed', 'disconnected');
  }
}
